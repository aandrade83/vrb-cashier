...
export default async function CashierDepositsPage({
  params,
}: {
  ...
  const [methods, player] = await Promise.all([
    ...
    getPlayerByClerkId(clerkId, cashierId),
  ]);
      ...
      {recentTx.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Link href={`${base}/transactions`} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
              View all →
            </Link>
          </CardHeader>
