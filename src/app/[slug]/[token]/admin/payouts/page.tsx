...
export default async function AdminPayoutsPage({
  ...
  const activeTab = (PAYOUT_TABS.find((t) => t.label === tab) ?? PAYOUT_TABS[1])!;
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 border-b pb-2 flex-wrap">
        {PAYOUT_TABS.map((t) => (
          <Link
            key={t.label}
            href={`${base}/payouts?tab=${encodeURIComponent(t.label)}`}
      ...
      {activeTab.label === "Search" && (
        <form method="get" action={`${base}/payouts`}>
          <input type="hidden" name="tab" value="Search" />
          <Input
            name="q"
            defaultValue={q ?? ""}
            placeholder="Search by reference, email, or player name..."
            className="max-w-md"
          />
        </form>
      )}
