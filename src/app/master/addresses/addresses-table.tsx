...
import {
  toggleAddressActiveAction,
  updateAddressPriorityAction,
  deleteAddressAction,
} from "./actions";
...
export function AddressesTable({ rows, cashierId }: Props) {
  ...
  function handleToggle(id: string) {
    startTransition(async () => {
      await toggleAddressActiveAction(id);
      router.refresh();
    });
  }
  ...
  async function handlePrioritySave(id: string) {
    ...
    const result = await updateAddressPriorityAction(id, p);
    if (!result.success) {
      setErrors((prev) => ({ ...prev, [id]: result.error }));
      return;
    }
  ...
  if (rows.length === 0) {
    return (
      <p className="text-center py-12 text-sm text-muted-foreground">
        No addresses in the pool yet. Add one above.
      </p>
    );
  }
            ...
            <TableCell className="text-right">
              <div className="flex justify-end gap-1">
                ...
                <Button
                  ...
                  title={row.isLocked ? "Cannot delete a locked address" : "Delete"}
                >
