...
export default async function ClerkDashboardPage() {
  ...
  const cashierIds = session.masterUserId
    ...
    : [];
  const allCashiers = cashierIds.length > 0 ? await getCashiersByIds(cashierIds) : [];
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {allCashiers.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              You don&apos;t have access to any cashier. Please verify permissions with Administration.
            </div>
          ) : (
